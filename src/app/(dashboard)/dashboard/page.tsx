...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/clients?pageSize=1");
        const data = await res.json();
        setStats({
          totalClients: data.data?.total ?? 0,
          noticesUnread: 0,
        });
      } catch {
        // ignore
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Your AI-powered practice management platform. Let us get started.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          ...
          {
            label: "Unreviewed Notices",
            value: loading ? "..." : stats.noticesUnread,
            color: "bg-red-50 text-red-700",
          },
          {
            label: "GST Recons Due",
            value: "—",
